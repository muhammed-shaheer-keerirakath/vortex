...
import { FinalResult, SrcIfc, compilationInterface, ASTInterface, Options, TestResultInterface, AstNode, CompilerConfiguration } from './types'
require('colors')
      ...
      [
          ...
          deployAll(compilationResult, this.web3, this.testsAccounts, false, deployCb, (err, contracts) => {
            if (err) {
              ...
              if (err.error.includes("The contract code couldn't be stored, please check your gas limit")) {
                ...
                next([{ message: 'contract deployment failed: ' + (err.innerError || err.error), severity: 'error' }])
        ...
        (contractsToTest: string[], contractsToTestDetails: any[], contracts: any, next) => {
          ...
          async.eachOfLimit(
            contractsToTest,
            1,
            (contractName: string, index: string | number, cb: ErrorCallback) => {
