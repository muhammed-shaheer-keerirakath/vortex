...
describe('Accounts', () => {
  ...
  describe('zond_getBalance', () => {
    it('should get a account balance', async () => {
      const accounts: string[] = await web3.zond.getAccounts()
      const balance0: string = await web3.zond.getBalance(accounts[0], undefined, { number: FMT_NUMBER.STR, bytes: FMT_BYTES.HEX })
      const balance1: string = await web3.zond.getBalance(accounts[1], undefined, { number: FMT_NUMBER.STR, bytes: FMT_BYTES.HEX })
      const balance2: string = await web3.zond.getBalance(accounts[2], undefined, { number: FMT_NUMBER.STR, bytes: FMT_BYTES.HEX })
  ...
  describe('zond_sign', () => {
    it('should sign payloads', async () => {
      const accounts: string[] = await web3.zond.getAccounts()
      const signature = await web3.zond.sign(web3.utils.utf8ToHex('Hello world'), accounts[0])

      assert.deepEqual(typeof signature === 'string' ? signature.length : signature.signature.length, 132)
    })
  })

  describe('zond_signTypedData', () => {
