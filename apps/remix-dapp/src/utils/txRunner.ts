...
export const shortenAddress = (address: string, etherBalance?: string) => {
  ...
  return address.slice(0, 5) + '...' + address.slice(len - 5, len) + (etherBalance ? ' (' + etherBalance.toString() + ' etherx)' : '')
}
...
export class TxRunner {
  ...
  setProvider(provider: string | SupportedProviders<ZondExecutionAPI> | undefined) {
    ...
    void web3.zond
      ...
      .then(async (accounts) => {
        ...
        for (const account of accounts) {
          const balance = await this.getBalanceInZnd(account)
  ...
  async getBalanceInZnd(address: string) {
  ...
  async _updateChainContext() {
    try {
      ...
      this.blockGasLimit = block?.gasLimit ? Math.floor(Number(web3.utils.toNumber(block.gasLimit)) - (5 * Number(web3.utils.toNumber(block.gasLimit))) / 1024) : web3.utils.toNumber(this.blockGasLimitDefault)
  ...
  async _executeTx(tx: any, network: any) {
    ...
    try {
      const { transactionHash } = await web3.zond.sendTransaction(tx, undefined, { checkRevertBeforeSending: false, ignoreGasPricing: true })
      ...
      console.log(`Send transaction failed: ${error.message || error.error} . if you use an injected provider, please check it is properly unlocked. `)
      return { error }
    }
  }
}
